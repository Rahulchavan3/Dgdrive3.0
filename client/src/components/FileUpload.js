...
const FileUpload = ({ contract, account, provider }) => {
  ...
  const handleSubmit = async (e) => {
    //to prevent from reloding when we choose  the file
    e.preventDefault();
    //for uploading file to pinata
    if (file) {
      try {
        const formData = new FormData();
        formData.append("file", file);
        //for uploading file to pinata this will store image on pinata ipfs 
        const resFile = await axios({
          method: "post",
          url: "https://api.pinata.cloud/pinning/pinFileToIPFS",
          data: formData,
          headers: {
            pinata_api_key: `b99bb7ef8e51396496db`,
            pinata_secret_api_key: `fcc8d74d7584ef9377064f8811b6f73b57a325434764d1e6fee1801baee8ff15`,
            "Content-Type": "multipart/form-data",
          },
        });

        //used to retrive image hash from pinta to access image
        const ImgHash = `https://gateway.pinata.cloud/ipfs/${resFile.data.IpfsHash}`;
        //used to store image hash on blockchain these add function we have created in solidity file
  ...
  //it helps to retrive image hash from pinta to access image
